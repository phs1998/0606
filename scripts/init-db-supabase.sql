...
-- 注意：Supabase已默认启用uuid-ossp扩展，使用gen_random_uuid()替代uuid_generate_v4()
...
-- 每日故事表索引
CREATE INDEX IF NOT EXISTS idx_daily_stories_date ON daily_stories(date);
CREATE INDEX IF NOT EXISTS idx_daily_stories_rarity ON daily_stories(rarity);
CREATE INDEX IF NOT EXISTS idx_daily_stories_category ON daily_stories(category);

-- ============================================
-- 4. 故事抽奖记录表 (story_draws)
-- ============================================
CREATE TABLE IF NOT EXISTS story_draws (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- 故事抽奖记录表索引
CREATE INDEX IF NOT EXISTS idx_story_draws_user_id ON story_draws(user_id);
CREATE INDEX IF NOT EXISTS idx_story_draws_story_id ON story_draws(story_id);
CREATE INDEX IF NOT EXISTS idx_story_draws_draw_date ON story_draws(draw_date);
CREATE INDEX IF NOT EXISTS idx_story_draws_user_date ON story_draws(user_id, draw_date);
...
DROP TRIGGER IF EXISTS update_daily_stories_updated_at ON daily_stories;
...
DROP TRIGGER IF EXISTS update_reply_count_on_insert ON messages;
...
DROP TRIGGER IF EXISTS update_reply_count_on_delete ON messages;
...
DROP TRIGGER IF EXISTS update_like_count_on_insert ON message_likes;
CREATE TRIGGER update_like_count_on_insert
    AFTER INSERT ON message_likes
    FOR EACH ROW
    EXECUTE FUNCTION update_message_like_count();

DROP TRIGGER IF EXISTS update_like_count_on_delete ON message_likes;
...
DROP TRIGGER IF EXISTS update_draw_count_on_insert ON story_draws;
CREATE TRIGGER update_draw_count_on_insert
    AFTER INSERT ON story_draws
    FOR EACH ROW
    EXECUTE FUNCTION update_story_draw_count();

DROP TRIGGER IF EXISTS update_draw_count_on_delete ON story_draws;
